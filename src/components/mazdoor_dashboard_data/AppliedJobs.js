...
import { fetchAppliedJobsByLaborId } from "../../store/reducers/jobSlice";

export default function AppliedJobs() {
  const dispatch = useDispatch();
  const { jobs, loading, error } = useSelector((state) => state.jobs);
  const laborId = useSelector((state) => state.auth.user.laborId); // Assuming laborId is stored in auth state

  useEffect(() => {
    if (laborId) {
      dispatch(fetchAppliedJobsByLaborId(laborId));
    }
  }, [laborId, dispatch]);
